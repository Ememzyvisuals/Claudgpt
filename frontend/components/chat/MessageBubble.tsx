...
import { CodeExportPanel } from '@/components/chat/CodeExportPanel';
        ...
        <AnimatePresence>
          ...
          {!editing && (
            ...
            >
              ...
              {/* Files export — show when AI message has code blocks */}
              {!isUser && !message.isStreaming && message.content.includes('```') && (
                <button
                  onClick={() => setShowExport(!showExport)}
                  className={cn("flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-semibold transition",
                    showExport ? "bg-[#C4A484] text-white" : "hover:bg-[#EDE8E2] text-[#A89585] hover:text-[#6B5D52]"
                  )}
